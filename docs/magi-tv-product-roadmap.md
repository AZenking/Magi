...
EPG 作为播放器内的全屏覆盖层打开，不离开当前播放页：

```text
全屏节目单网格
→ 左侧固定频道栏（台标、频道号、名称、收藏、播放状态）
→ 右侧 30 分钟时间轴和按时长计算的节目块
→ 默认显示当前时间前后 2 小时
→ 日期栏支持未来 7 天，分组/收藏/最近使用横向滚动
→ OK 播放当前、历史或未来节目所属频道
→ Back / 左键关闭并恢复原频道、焦点和播放位置
```
...
- 当前时间前后 2 小时的网格浏览。
- 7 天日期切换，并保持相同本地时间段。
- 当前节目高亮、当前时间线、节目加载/过期/错误/空状态。
- 只支持浏览和播放，不实现节目详情、Catch-up、录制或提醒。

后续阶段再评估节目详情、回看、录制和提醒，避免第一阶段改变播放器主流程。
