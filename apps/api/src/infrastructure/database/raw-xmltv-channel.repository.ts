...
export class RawXmltvChannelRepository implements IRawXmltvChannelRepository {
  ...
  async findCandidates(params: {
    ...
    page: number;
    pageSize: number;
  }): Promise<{ items: RawXmltvChannel[]; total: number }> {
    const conditions = [];
    if (params.sourceId)
      conditions.push(eq(rawXmltvChannels.sourceId, params.sourceId));
    if (params.search) {
      const term = `%${params.search}%`;
      conditions.push(
        or(
          ilike(rawXmltvChannels.xmltvId, term),
          ilike(rawXmltvChannels.displayName, term),
        )!,
      );
    }
  ...
  async createBatch(
    channels: Omit<RawXmltvChannel, "id" | "createdAt" | "updatedAt">[],
  ): Promise<RawXmltvChannel[]> {
