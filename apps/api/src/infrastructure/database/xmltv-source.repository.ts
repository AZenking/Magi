...
export class XmltvSourceRepository implements IXmltvSourceRepository {
  ...
  async findPaginated(
  ...
  ): Promise<PaginatedSourcesResult<XmltvSource>> {
    ...
    if (search) {
      ...
      conditions.push(
        or(
          ilike(xmltvSources.name, `%${escaped}%`),
          ilike(xmltvSources.url, `%${escaped}%`),
        )!,
      );
    }
  ...
  async update(id: string, data: XmltvUpdateData): Promise<XmltvSource | null> {
    const [row] = await db
      ...
      .set({
        ...data,
        version: sql`${xmltvSources.version} + 1`,
        updatedAt: new Date(),
      })
      .where(eq(xmltvSources.id, id))
      .returning();
  ...
  async updateIfVersion(
    id: string,
    data: XmltvUpdateData,
    expectedVersion: number,
  ): Promise<XmltvSource | null> {
    const [row] = await db
      .update(xmltvSources)
      .set({ ...data, version: expectedVersion + 1, updatedAt: new Date() })
      .where(
        and(eq(xmltvSources.id, id), eq(xmltvSources.version, expectedVersion)),
      )
      .returning();
    return row ? toDomain(row) : null;
  }
