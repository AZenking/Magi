  ...
  implements IOperationExecutionRepository, ICleanupPort
{
  ...
  async loadSnapshotFingerprint(snapshotId: string): Promise<string | null> {
    const [row] = await db
      .select({ contentFingerprint: sourceImportSnapshots.contentFingerprint })
      .from(sourceImportSnapshots)
      .where(eq(sourceImportSnapshots.id, snapshotId))
      .limit(1);
    return row?.contentFingerprint ?? null;
  }

  async finalizeChangeSet(
    changeSetId: string,
    status: string,
    version: number,
  ): Promise<boolean> {
  ...
  async releaseLease(scopeKey: string, taskId: string): Promise<void> {
    await db
  ...
  async expireTerminalChangeSets(cutoff: Date): Promise<number> {
    ...
    for (const c of candidates) {
      ...
      const [rp] = await db
        ...
        .from(recoveryPoints)
        .where(eq(recoveryPoints.changeSetId, c.id))
        .limit(1);
      if (rp) continue;
      const [ae] = await db
        .select({ id: auditEvents.id })
        .from(auditEvents)
        .where(eq(auditEvents.changeSetId, c.id))
        .limit(1);
      if (ae) continue;
      await db
        .delete(operationChangeItems)
        .where(eq(operationChangeItems.changeSetId, c.id));
      await db
        .delete(operationChangeSets)
        .where(eq(operationChangeSets.id, c.id));
      deleted++;
    }
    return deleted;
  }
  ...
  async expireSnapshots(cutoff: Date): Promise<number> {
    ...
    for (const s of candidates) {
      ...
      const [cs] = await db
        .select({ id: operationChangeSets.id })
        .from(operationChangeSets)
        .where(eq(operationChangeSets.snapshotId, s.id))
        .limit(1);
      if (cs) continue;
      await db
        .delete(sourceImportSnapshotItems)
        .where(eq(sourceImportSnapshotItems.snapshotId, s.id));
      await db
        .delete(sourceImportSnapshots)
        .where(eq(sourceImportSnapshots.id, s.id));
      deleted++;
    }
    return deleted;
  }
  ...
  async reclaimExpiredLeases(now: Date): Promise<number> {
    const expired = await db
      .select()
      .from(operationLeases)
      .where(lt(operationLeases.expiresAt, now));
    let reclaimed = 0;
    for (const lease of expired) {
      if (!lease.taskId) {
        await db
          .delete(operationLeases)
          .where(eq(operationLeases.scopeKey, lease.scopeKey));
        reclaimed++;
        continue;
      }
      ...
      const [task] = await db
        ...
        .from(syncLogs)
        .where(eq(syncLogs.id, lease.taskId))
        .limit(1);
